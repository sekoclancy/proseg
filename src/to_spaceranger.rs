...
fn main() {
    ...
    let geometries = read_proseg_to_baysor_cellpolygon_geojson(&args.input_cell_polygons);
...
fn read_proseg_to_baysor_cellpolygon_geojson(filename: &str) -> Vec<JsonValue> {
    let file = File::open(filename).unwrap();
    let json_str = std::io::read_to_string(file).unwrap();
    let parsed = json::parse(&json_str).unwrap();
